...
class _PlayCardState extends State<PlayCard> {
  ...
  Widget _buildFlipAnimation(context) {
    // int index = 1;
    return BlocBuilder<PlayCardBloc, PlayCardState>(
      builder: (context, state) {
        return GestureDetector(
          onTap: () {
            setState(() {
              context
                  .read<PlayCardBloc>()
                  .add(FlipPlayCardEvent(widget.brains.index));
          ...
          child: AnimatedSwitcher(
            ...
            child: widget.brains.isFlippedUp ? _buildRear() : _buildFront(),
          ),
        );
      },
    );
  ...
  Widget __transitionBuilder(Widget widget, Animation<double> animation) {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, widget) {
        final isUnder =
            (ValueKey(super.widget.brains.isFlippedUp) != widget?.key);
  ...
  Widget _buildFront() {
    return __buildLayout(
      ...
      backgroundColor: widget.brains.isMatched
          ? Colors.white
          : widget.brains.index == -2
              ? Colors.white
              : Colors.blue,
